...
class Social extends StatelessWidget {
  ...
  final List<DocumentSnapshot> noticiasSocial;
  
  Social(this.noticiasSocial);
  ...
  Widget build(BuildContext context) {
    ...
    return StaggeredGridView.count(
      ...
      crossAxisSpacing: 5,
      crossAxisCount: 2,
      staggeredTiles: noticiasSocial.map((noticia) {
        if (noticia['tipo_noticia'] == 'social') {
          return StaggeredTile.count(
              noticia['largura_capa'], noticia['altura_capa']);
        } else
          return null;
      }).toList(),
      children: noticiasSocial.map((noticia) {
        if (noticia['tipo_noticia'] == 'social') {
          return GestureDetector(
            ...
            onTap: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => Noticia(noticia)));
            },
          );
        } else
          return null;
